export default class NMModal {
      ...
      if (this.modal) {
         document.addEventListener('click', function (e) {
            ...
            if (clickedElement) {
               ...
               let delay = clickedElement.dataset.modalDelay;
               this.animation = animation ? animation : 'fade';
               this.speed = speed ? parseInt(speed) : 300;
               this.delay = delay ? parseInt(delay) : 300;
         ...
         document.addEventListener('click', function (e) {
            if (e.target.classList.contains('modal') && e.target.classList.contains("_open")) {
               this.close();
            }
         }.bind(this));
      }
   ...
   open(selector) {
      ...
      this.modalContainer.classList.add('_modal-open');
      this.modalContainer.classList.add(this.animation);

      setTimeout(() => {
         this.options.isOpen(this);
         this.modalContainer.classList.add('animate-end');
         this.isOpen = true;
         this.focusTrap();
      }, this.delay);
   }
   ...
   close() {
      if (this.modalContainer) {
         this.modalContainer.classList.remove('animate-end');
         this.modalContainer.classList.remove(this.animation);
         this.modal.classList.remove('_open');
         this.modalContainer.classList.remove('_modal-open');
